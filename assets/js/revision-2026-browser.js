...
(() => {
  ...
  function subjectCard(subject, programmeName) {
    ...
    return `<article class="subject-card" data-subject-code="${esc(code.toUpperCase())}" data-revision="2026" data-semester="${esc(semesterText)}" data-search-text="${esc([code, name, programmeName, semesterText, type].join(" ").toLowerCase())}" data-notes-href="${esc(notes)}" data-lesson-href="${esc(lesson)}" data-lesson-available="true" data-notes-available="true"><div class="subject-top"><span>2026</span><strong>${esc(code)}</strong></div><h3>${esc(name)}</h3><p>${esc(meta)}</p><div class="action-row"><a class="action syllabus" href="${esc(syllabus)}" target="_blank" rel="noopener noreferrer">Open Syllabus</a><a class="action lessons" href="${esc(lesson)}">View Lessons</a><a class="action download" href="${esc(notes)}" title="Open the lesson in print mode and save it as a PDF.">Download Notes</a><button class="action qp" type="button" data-model-paper-unavailable="true" data-model-paper-revision="2026" data-model-paper-course="${esc(code.toUpperCase())}" aria-label="${esc(qpMessage)}" title="${esc(qpMessage)}" onclick="window.alert(this.title)">Sample Question Paper</button></div></article>`;
  }
