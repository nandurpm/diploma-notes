...
(() => {
  ...
  function fields(defs, calc, note=''){
    ...
    /* Form submit intercept to run calculations without full page reload */
    $('#toolForm').onkeydown = e => { if (e.key === 'Enter') { e.preventDefault(); $('#calcBtn').click(); } };
  }
  ...
  function textTool(kind){ $('#body').innerHTML=`<label class='sr-only' for='text'>Text</label><textarea id='text' rows='9' placeholder='Paste or type text here'></textarea><div class='tool-actions'><button class='btn primary' id='run' type='button'>Run Tool</button></div><div class='result' id='res' role='status' aria-live='polite'>Ready.</div>`;
    $('#text').onkeydown = e => { if ((e.ctrlKey || e.metaKey) && e.key === 'Enter') { e.preventDefault(); $('#run').click(); } };
