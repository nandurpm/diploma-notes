...
(()=>{
  ...
  function card(s){const r=root(),ac=asset(s),les=`${r}lessons/lessons-${encodeURIComponent(ac)}.html`,print=`${les}?autoPrintNotes=1`,has=LESSONS.has(code(ac));return `<article class='subject-card' data-subject-code='${esc(code(s.code))}' data-revision='${esc(s.revision)}' data-notes-href='${esc(print)}' data-lesson-href='${esc(les)}' data-lesson-available='${has}' data-notes-available='${has}'><div class='subject-top'><span>${esc(s.revision)}</span><strong>${esc(s.code)}</strong></div><h3>${esc(s.name)}</h3><p>${esc(s.department)} / ${esc(s.semester)} / ${esc(s.type)}</p><div class='action-row'><a class='action syllabus' href='${esc(syllabus(s))}' target='_blank' rel='noopener noreferrer'>Open Syllabus</a>${has?`<a class='action lessons' href='${esc(les)}'>View Lessons</a><a class='action download' href='${esc(print)}' title='Open the lesson in print mode and save it as a PDF.'>Download Notes</a>`:`<span class='availability-label'>Lessons unavailable</span><span class='availability-label'>Notes unavailable</span>`}${qp(s)?`<a class='action qp' href='${esc(qp(s))}' target='_blank' rel='noopener noreferrer' data-model-paper-revision='${esc(s.revision)}' data-model-paper-course='${esc(code(s.code))}'>Sample QP</a>`:`<button class='action qp' type='button' data-model-paper-unavailable='true' data-model-paper-revision='${esc(s.revision)}' data-model-paper-course='${esc(code(s.code))}' aria-label='Model Question Paper not available for Revision ${esc(s.revision)} for course ${esc(s.code)}.' title='Model Question Paper not available for Revision ${esc(s.revision)} for course ${esc(s.code)}.' onclick='window.alert(this.title)'>Sample QP</button>`}</div></article>`}
