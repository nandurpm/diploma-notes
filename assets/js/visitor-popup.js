...
(() => {
  ...
  function installStyles() {
    ...
    style.textContent = `
      .poly-media-popup-backdrop{position:relative;display:none;margin:24px auto;max-width:720px;padding:0 16px}
      .poly-media-popup-backdrop.open{display:flex}
      .poly-media-popup{width:100%;overflow:hidden;border:1px solid rgba(255,255,255,.78);border-radius:28px;background:#fff;box-shadow:0 32px 100px rgba(15,23,42,.32);font-family:Inter,system-ui,-apple-system,Segoe UI,sans-serif;animation:polyPopupIn .22s ease both}
  ...
  function mediaMarkup(item) {
    if (item.type === "video") return `<video src="${item.src}" controls muted playsinline preload="metadata"></video>`;
  ...
  function openPopup(item, index) {
    ...
    const close = () => {
      const hadFocus = backdrop.contains(document.activeElement);
      backdrop.remove();
      if (hadFocus) {
    ...
    // Keep updates available without taking focus, blocking study content, or
    // imposing a time limit on reading. Listeners disappear with the notice.
    const footer = document.querySelector('footer');
    if (footer) footer.before(backdrop);
    else document.body.append(backdrop);
