...
(() => {
  ...
  async function data() { let base = Array.isArray(globalThis.SUBJECTS) ? globalThis.SUBJECTS : []; const manifest = await fetch(`${root()}assets/data/sitttr-pdf-links.json?v=${VERSION}`, {cache:"no-store"}).then(r => r.ok ? r.json() : null).catch(()=>null); PDF_LINKS = manifest?.links?.["2021"] || {}; if (!base.length) { const text = await fetch(`${root()}assets/js/subjects.js?v=${VERSION}`, {cache:"no-store"}).then(r => r.ok ? r.text() : "").catch(()=>""); base = parse(text); } const merged = uniq([...base, ...supplemental]); /* PERFORMANCE OPTIMIZATION: Pre-compute and cache search text for each subject to avoid redundant string joins and lowercase conversions on every keystroke. */ merged.forEach(s => { s._searchText = [s.code, s.name, s.department, s.semester, s.type].join(" ").toLowerCase(); }); return merged; }
  ...
  function pdfKey(s) { const department = String(s.department || "").toLowerCase().replaceAll("&", " and ").replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, ""); return `2021|${department}|${norm(s.code)}`; }
  function pdfHref(s, kind) { const path = PDF_LINKS[pdfKey(s)]?.[kind]; return path ? `${PDF_BASE}${path}` : ""; }
  function sitttrModelPaperFallback() { return `${SITTTR_BASE}r=site%2Fdiploma-modelqp&scheme=REV2021`; } function sitttrHref(s, kind) { const code = encodeURIComponent(norm(s.code)); return kind === "modelQuestionPaper" ? sitttrModelPaperFallback() : `${SITTTR_BASE}r=site%2Fdiploma-syllabus-course-contents&course=${code}&scheme=REV2021`; }
  function pdfName(href) { return href.split("/").pop() || "syllabus.pdf"; }
  function card(s) { const r = root(), ac = asset(s), lesson = `${r}lessons/lessons-${encodeURIComponent(ac)}.html`, print = `${lesson}?autoPrintNotes=1`, ok = hasLesson(s), meta = [displayDept(s.department), s.semester, s.type].filter(Boolean).join(" / "), syllabusHref = pdfHref(s, "syllabus"), qpHref = pdfHref(s, "modelQuestionPaper"), syllabusAction = syllabusHref ? `<a class="action syllabus" href="${esc(syllabusHref)}" download="${esc(pdfName(syllabusHref))}">Download Syllabus</a>` : `<a class="action syllabus external-fallback" href="${esc(sitttrHref(s, "syllabus"))}" target="_blank" rel="noopener noreferrer">Open SITTTR Syllabus</a>`, qpAction = qpHref ? `<a class="action qp" href="${esc(qpHref)}" download="${esc(pdfName(qpHref))}">Download Model Question Paper</a>` : `<a class="action qp external-fallback" href="${esc(sitttrHref(s, "modelQuestionPaper"))}" target="_blank" rel="noopener noreferrer">Open SITTTR Model Question Paper \u2014 REV2021</a>`, study = ok ? `<a class="action lessons" href="${esc(lesson)}">View Lessons</a><a class="action download" href="${esc(print)}">Save as PDF</a>` : `<span class="availability-label lessons-status" aria-disabled="true">Lessons unavailable</span><span class="availability-label notes-status" aria-disabled="true">Notes unavailable</span>`; return `<article class="subject-card" data-subject-code="${esc(norm(s.code))}" data-revision="${esc(s.revision)}" data-semester="${esc(s.semester)}" data-search-text="${esc([s.code, s.name, s.department, s.semester, s.type].join(" ").toLowerCase())}" data-notes-href="${esc(print)}" data-lesson-href="${esc(lesson)}" data-lesson-available="${String(ok)}" data-notes-available="${String(ok)}"><div class="subject-top"><span>${esc(s.revision)}</span><strong>${esc(s.code)}</strong></div><h3>${esc(s.name)}</h3><p>${esc(meta)}</p><div class="action-row">${syllabusAction}${study}${qpAction}</div></article>`; }
