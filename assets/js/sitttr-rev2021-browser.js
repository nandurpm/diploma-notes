...
(() => {
  ...
  // Direct per-course link only for codes with no REV2026 collision (see
  // REV2021_SAFE_CODES). A colliding code can't be disambiguated by the
  // government site, so it falls back to the REV2021 scheme index instead
  // of risking a link to the wrong revision's content.
  function syllabus(s) { const code = norm(asset(s)); return REV2021_SAFE_CODES.has(code) ? `https://www.sitttrkerala.ac.in/index.php?r=site%2Fdiploma-syllabus-course-contents&course=${encodeURIComponent(asset(s))}` : "https://www.sitttrkerala.ac.in/index.php?r=site%2Fdiploma-syllabus&scheme=REV2021"; }
