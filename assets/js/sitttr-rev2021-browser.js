...
(() => {
  ...
  function card(s) { const r = root(), ac = asset(s), lesson = `${r}lessons/lessons-${encodeURIComponent(ac)}.html`, print = `${lesson}?autoPrintNotes=1`, ok = hasLesson(s), meta = [displayDept(s.department), s.semester, s.type].filter(Boolean).join(" / "), syllabusHref = pdfHref(s, "syllabus"), qpHref = pdfHref(s, "modelQuestionPaper"), syllabusAction = syllabusHref ? `<a class="action syllabus" href="${esc(syllabusHref)}" download="${esc(pdfName(syllabusHref))}">Download Syllabus</a>` : `<span class="availability-label syllabus-status" aria-disabled="true">Syllabus unavailable</span>`, qpAction = qpHref ? `<a class="action qp" href="${esc(qpHref)}" download="${esc(pdfName(qpHref))}">Download Model Question Paper</a>` : `<span class="availability-label qp-status" aria-disabled="true">Model paper unavailable</span>`; return `<article class="subject-card" data-subject-code="${esc(norm(s.code))}" data-notes-href="${esc(print)}" data-lesson-href="${esc(lesson)}" data-lesson-available="${ok}" data-notes-available="${ok}"><div class="subject-top"><span>${esc(s.revision)}</span><strong>${esc(s.code)}</strong></div><h3>${esc(s.name)}</h3><p>${esc(meta)}</p><div class="action-row">${syllabusAction}${ok ? `<a class="action lessons" href="${esc(lesson)}">View Lessons</a><a class="action download" href="${esc(print)}" title="Open the lesson in print mode and save it as a PDF.">Save as PDF</a>` : `<span class="availability-label lessons-status" aria-disabled="true">Lessons unavailable</span><span class="availability-label notes-status" aria-disabled="true">Notes unavailable</span>`}${qpAction}</div></article>`; }
