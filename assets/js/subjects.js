// SUBJECTS: If two entries share the same code they are treated as common subjects.
// Only one card is shown (first match) with department = "First Year / Common".
// Add lessonFile: "lessons/lessons-XXXX.html" to link the View Lessons button to a real page.
// Add notesFile:  "notes/downloadable-notes-XXXX.pdf"  to show a Download Notes button.
const SUBJECTS = [
  // ── First Year / Common ──────────────────────────────────────────────────
  ...
  { revision: "2021", code: "1004", name: "Applied Chemistry",                        department: "First Year / Common", semester: "Semester 1", type: "Theory",
    lessonFile: "lessons/lessons-1004.html",
    notesFile:  "notes/downloadable-notes-1004.pdf" },
...
function notesLink(subject) {
  if (!subject.notesFile) return null;
  const depth = window.location.pathname.replace(/\/[^/]*$/, "").split("/").filter(Boolean).length;
  const prefix = depth > 0 ? "../".repeat(depth) : "";
  return prefix + subject.notesFile;
}
