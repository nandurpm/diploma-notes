...
EXCLUDED_ROOTS = {
    ...
    "maintenance", "tools", "workers", "node_modules", "_site",
}
...
def build(target: Path, optimize: bool) -> None:
    ...
    # Normalize the staged artifact, not only the source tree. This prevents
    # mixed page/asset revisions when a checkout contains HTML generated by
    # earlier runs or when CI does not mutate tracked source files.
    build_info = json.loads((target / "build-info.json").read_text(encoding="utf-8"))
    build_id = str(build_info.get("buildId", "")).strip()
    if not build_id:
        raise ValueError("build-info.json does not contain a buildId")
    subprocess.check_call(
        ["python", str(ROOT / "tools/inject_build_id.py"), build_id, "--root", str(target)],
        cwd=ROOT,
    )
