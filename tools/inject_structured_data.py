...
def public_pages() -> list[Path]:
    ...
    for path in ROOT.rglob("*.html"):
        ...
        if relative.parts and relative.parts[0] in EXCLUDED_ROOTS:
            continue
        if path.name in {"department-view.html", "tools-v2-original.html", "new-year-theme-preview.html", "404.html"}:
            continue
        result.append(path)
...
def main() -> int:
    ...
    if args.check and changed:
        ...
        return 1
    print(f"{'Verified' if args.check else 'Generated'} structured data on {len(public_pages())} public pages; changed {len(changed)}.")
    return 0
