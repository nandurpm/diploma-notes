...
-- POLY PMNA owner account can approve and publish them.
...
-- Stable auth user id for the site owner. Keeping the approval decision in the
-- database means a client-side button cannot bypass it.
...
as $$
  select auth.uid() = '35c194b6-4265-481d-addb-db55bfcb5a8f'::uuid;
$$;
...
-- Returns contributor identity only to the owner review UI. Email is not stored
-- on blog_posts and therefore is never exposed to public readers.
...
-- Public reading stays unchanged. Contributors can upload only inside their
-- auth user-id folder; the owner can manage all blog images.
