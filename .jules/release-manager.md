...
## 2026-07-31 - Compiling and Verifying Static Deployment Builds

**Learning:** Deploying compiled public builds without comprehensive verification risks shipping broken optimized assets, missing critical pages/configs, or failing quiet background assertions (like CSS bundling on `index.html` or about page bilingual structures). Extending the central quality gate (`tools/site_quality_gate.py`) to support a `--build-dir` argument enables identical, rigorous, automated validation of actual compiled target folders (`_site/`) in local diagnostic runs and GHA pipelines.

**Action:** Always verify local and pipeline static builds by running `python tools/site_quality_gate.py --build-dir _site` right after `build_public_site.py` to ensure that compiled public artifacts are complete, correctly optimized, and structurally sound before deployment.
