...
## 2026-07-31 - [Cached Textbook Sections and textContent Caching in Engineering Handbook]
**Learning:** Discovered a massive layout-thrashing bottleneck inside the engineering handbook's active keypress search listener. It queried the entire DOM for all sections (`.hb-hero, .hb-section`) on every keystroke and called `.innerText`, triggering costly layout recalculations (browser reflows) and O(N) DOM querying on every character typed. Replacing `.innerText` with cached `.textContent` values pre-computed on initial render reduces the search cost to O(1) DOM query and lightweight O(N) string containment checks with zero reflow penalty.
**Action:** Avoid calling `.innerText` or performing document-wide querying inside active user input handlers; query once at setup-time and use `.textContent` or cached properties to prevent browser reflows.
