...
The production API is exposed through the WAF-protectable custom Worker hostname `api.polypmna.dpdns.org`; browser clients and release probes use this hostname rather than the bypassable `workers.dev` hostname. A Cloudflare custom firewall ruleset named `POLY PMNA API edge guard` blocks undocumented paths and permits only `/health`, `/api/ask-poly`, `/api/evaluate-mock-exam`, and `/api/grade-daily-quiz`. Cloudflare’s free-zone rate-limit allowance remains occupied by the existing leaked-credential rule, so the Worker’s distributed limiters remain authoritative for API quotas. Do not rely on a user-agent block as the sole bot defense because user agents are trivial to spoof.
...
| Ask POLY API | WAF-protected custom hostname, documented-path edge guard, distributed request limiter, local fallback limiter, origin allowlist, request-size cap | Add Cloudflare rate-limit/Bot Management or Turnstile when plan capacity or abuse metrics justify it |
...
| Image generation | Dedicated distributed two-per-minute limit plus local ten-minute budget and burst limiter | Stronger challenge or authenticated quota if image generation is exposed publicly |
...
The release gate cannot rotate a credential that already exists in Google/Firebase history or change Supabase and Cloudflare dashboard settings. Those actions remain explicit production sign-off tasks: rotate the historical Firebase key, apply the ownership migration, enable Supabase Auth abuse controls, configure stronger Cloudflare rate-limit/Bot Management or Turnstile controls if plan capacity permits, configure Workers Logs retention and alerts, and confirm database network restrictions.
